...
<div class="content-wrapper interface-blur">
    <div class="row">
        ...
        <div class="col-md-12 heading">
            ...
            <ol class="breadcrumb">
                <li>
                    <?= $this->Html->link(__d('admin', '{0} Dashboard', '<i class="fa fa-dashboard"></i>'),
                    ['controller' => 'admin',
                    'action' => 'home', 'prefix' => 'admin'], ['escape' => false]) ?>
                </li>
                <li>
                    <?= $this->Html->link(__d('admin', '{0} Manage Groups', '<i class="fa fa-newspaper-o"></i>'),
                    ['controller' => 'groups',
        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <div class="form-group">
                        <?= $this->Form->label('is_staff', __d('admin', 'Is Staff'), ['class' => 'col-sm-2
                        control-label']) ?>
                    ...
                    <div class="form-group">
                        <?= $this->Form->label('is_member', __d('admin', 'Is Member'), ['class' => 'col-sm-2
                        control-label']) ?>
                        <div class="col-sm-5 radio-check">
        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <h2>Public's Actions</h2>
                    <?php foreach($permissions['public'] as $controller => $actionList):?>
                    <h4><?= $controller;?></h4>
                    <div class="form-group">
                        <?= $this->Form->label($controller, __d('admin', $controller), ['class' => 'col-sm-2
                        control-label']) ?>
                        <?php foreach($actionList as $path => $actions):?>
                            <?php foreach($actions as $action):?>
                                <?php ($this->Acl->checkGroup($group, 'App/'.$path.'/'.$action) ? $check = ['val' =>1, 'class'
                                => 'primary'] : $check = ['val' =>0, 'class' => 'danger']);
                                ?>
                                <div class="col-sm-1 radio-check">
                                    <div class="text-<?=$check['class'];?> text-center text-uppercase">
                                        <?= __d('admin', $action) ?>
                                    </div>
                                    <?= $this->Form->select('App/'.$path.'/'.$action, [
                                    '1' => __d('admin', 'Yes'),
                                    '0' => __d('admin', 'No')
                                    ],
                                    [
                    ...
                    <?php endforeach;?>
                    <hr/>
                    <h2>Admin's Actions</h2>
                    <?php foreach($permissions['Admin'] as $controller => $actionList):?>
                    <h4><?= $controller;?></h4>

                    <div class="form-group">
                        <?= $this->Form->label($controller, __d('admin', 'actions'), ['class' => 'col-sm-2
                        control-label']) ?>
                        <?php foreach($actionList as $path => $actions):?>
                            <?php foreach($actions as $action):?>
                                <?php ($this->Acl->checkGroup($group, 'App/'.$path.'/'.$action) ? $check = ['val' =>1, 'class'
