...
<div class="robotpage-bottom-half">
    <div>
        <p id="robot-text">
            <?= __("Our monkey's team are working on this error !") ?>
        </p>
        <div class="robot-buttons">
            <?= $this->Html->link(__('Home'), ['controller' => 'pages', 'action' => 'home', 'prefix' => false], ['class' => 'btn btn-primary']) ?>
            <?= $this->Html->link(__('Contact'), ['controller' => 'contact', 'action' => 'index', 'prefix' => false], ['class' => 'btn btn-primary']) ?>
        </div>
    </div>
</div>
